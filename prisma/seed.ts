 ...
 * с лендингом). Критерии оценки — из официальных приложений к положению премии
 * (PDF). Каждый критерий: label → {key, label, maxScore:10, weight:1, step:0.5}.
 * Суперадмин может менять их в /admin/super/nominations без деплоя.
 */
const raw: {
  title: string;
  ...
  // 1.1 Организации / работодатели — Приложение №1
  {
    ...
    criteria: [
      "Трудовые проекты организации и их география",
      "Количество трудоустроенных в текущем календарном году участников трудовых отрядов подростков",
      "Медианная заработная плата у бойцов трудовых отрядов подростков",
      "Опыт организации социальных проектов во взаимодействии с трудовыми отрядами подростков",
      "Уникальные проекты и механика взаимодействия с трудовыми отрядами подростков",
    ],
  },
  // Приложение №2
  {
    ...
    criteria: [
      "Количество трудоустроенных в текущем календарном году участников студенческих отрядов",
      "Система управления охраной труда при трудоустройстве участников студенческих отрядов",
      "Механизм сопровождения участников в период трудового проекта",
      "Механизм профилактики производственного травматизма",
      "Результативность системы охраны труда (качественные и количественные показатели)",
      "Мероприятия по формированию культуры безопасного труда",
    ],
  },
  {
    title:
    ...
    criteria: [
      "Масштаб и охват деятельности студотрядов в вузе",
      "Система поддержки и развития студотрядов",
      "Достижения и результаты деятельности",
      "Взаимодействие с работодателями и партнёрами",
    ],
  },
  {
    title:
    ...
    criteria: [
      "Масштаб и охват деятельности студотрядов",
      "Система поддержки и развития студотрядов",
      "Достижения и результаты деятельности",
      "Взаимодействие с работодателями и партнёрами",
    ],
  ...
  // Приложение №5
  {
    ...
    criteria: [
      "Наличие и уровень подписанных соглашений, системность взаимодействия, создание совместной инфраструктуры",
      "Количество и описание социально-значимых мероприятий совместно с МООО «РСО»",
      "Организованная система наставничества со стороны кадровых сотрудников компании",
      "Количество и описание трудовых проектов всероссийского/окружного/межрегионального уровней совместно с МООО «РСО»",
      "Публикации и упоминания совместной деятельности в инфоресурсах и СМИ",
      "Программы финансовой поддержки (спонсорская помощь, образовательные программы, благотворительность)",
      "Наличие мультиформатного пространства «СО.здание» / коворкинга / штаба",
    ],
  },
  // 1.2 Региональные отделения — Приложение №6
  {
    ...
    criteria: [
      "Наличие официального документа деятельности совета ветеранов",
      "Количество и описание мероприятий, организованных советом ветеранов",
      "Участие во Всероссийской акции «День ударного труда» 2026",
      "Количество ветеранов советского периода в субъекте РФ",
      "Количество ветеранов и выпускников современного движения",
      "Механизм работы с выпускниками МООО «РСО» в региональном отделении",
      "Вклад в формирование музеев, экспозиций, стендов о студотрядах",
    ],
  },
  // Приложение №10
  {
  ...
  // Приложение №9/1
  {
    ...
    criteria: [
      "Опыт работы в движении студенческих отрядов",
      "Социальная и профессиональная активность по популяризации движения",
      "Проекты, реализованные по инициативе / под руководством, и их результаты",
      "Видеовизитка — раскрытие опыта и темы лидерства (до 1 мин)",
      "Портфолио — полнота и качество представленных материалов",
    ],
  },
  // 1.3 Органы власти — Приложение №8
  {
    ...
    criteria: [
      "Наличие действующего соглашения между МООО «РСО» и Правительством региона",
      "Наличие регионального закона о студенческих отрядах",
      "Наличие здания студенческих отрядов в регионе",
      "Наличие программы поддержки трудоустройства трудовых отрядов подростков",
      "Программы финансовой поддержки студенческих отрядов в регионе",
      "Меры нематериальной поддержки студенческих отрядов в регионе",
    ],
  },
  // Приложение №7
  {
    ...
    criteria: [
      "Наличие региональной программы поддержки ТОП",
      "Объём финансирования ТОП из бюджета субъекта РФ",
      "Количество созданных рабочих мест для подростков (через ТОП)",
      "Количество трудовых отрядов подростков на территории РФ",
      "Поддержка приёма участников ТОП из других субъектов РФ",
      "Меры нематериальной поддержки трудовых отрядов подростков",
      "Меры поддержки работодателям (субсидии, гранты, льготы, методпомощь)",
    ],
  },
  // 1.4 СМИ — Приложения №3–5 (заявка), №15–17 (оценочный лист)
  {
  ...
  // Приложение №11 — «Лидер студенческих отрядов», вариант для командиров/комиссаров трудовых проектов
  {
...
const FIELDS: Record<string, Field[]> = {
  ...
  // Высшее образование — Приложение №1 + справки (сканы PDF)
  "«Лучшая практика организации деятельности студотрядов в образовательной организации высшего образования»": [
    ...
    { name: "infoSheet1Scan", label: "Информационная справка №1 (Приложение №2) — скан в формате PDF", type: "file", required: true },
  ...
  "«Едины делом: Трудовой сезон РСО в объективе»": [
    ...
    { name: "authorEmail", label: "Электронная почта", type: "text", required: true },
    { name: "workTitle", label: "Название работы", type: "text", required: true },
    { name: "materialLink", label: "Ссылка на материалы, представляемые для участия в Национальной премии", type: "url", required: true },
  ],
  // Приложение №11
  "«Лидер студенческих отрядов» (командиры и комиссары трудовых проектов)": [
    { name: "fio", label: "Ф.И.О. (полностью)", type: "text", required: true },
    { name: "region", label: "Субъект Российской Федерации", type: "text", required: true },
    { name: "birthDate", label: "Дата рождения", type: "text", required: true },
